...
class Tracker:
    def __init__(self, frame_0, treshold=200):
        ...
        self.quality = 0.03
        self.max_corners = max(500, min(2000, round(
            self.frame_0.shape[0] / self.block_s *
            self.frame_0.shape[1] / self.block_s
        )))
    ...
    def find_flow(self, frame_1):
        ...
        new_corners, new_sizes = self.find_corners(
            frame_1, mask=self.get_corner_cover(tracked[m],
                                                self.corner_sizes[new_m])
        )
