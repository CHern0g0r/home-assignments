...
class CamTracker:
    def __init__(self, length, intrinsic_mat, corner_storage, v1, v2,
                 triang_params, retriang_params):
        ...
        corr = build_correspondences(
            self.corner_storage[v1[0]],
            self.corner_storage[v2[0]]
        )
    ...
    def retriangulatible_corners(self, frame, it):
        ...
        for ind in self.corner_storage[frame].ids.flatten():
            if (ind not in self.retriangulated or
                    self.retriangulated[ind] < it - 5):
                retr_candidates += [ind]
    ...
    def run_find_pose(self, refind=False):
        if refind:
            frames = [i for i in range(self.length)
                      if self.view_mats[i] is not None]
        else:
            frames = [i for i in range(self.length)
    ...
    def track(self):
        ...
        while self.empty_view > 0:
            ...
            if it % 5:
                ...
                if (self._inliers is not None and
                        new_fr is not None and
                        self._inliers[new_fr] < new_inl):
...
def track_and_calc_colors(camera_parameters: CameraParameters,
    ...
    retriang_params = TriangulationParameters(
        1.5, 2.5, 0.1
    )
